...
//        DamLevelConsumer().Consume()
//        FloodGateConsumer().Consume()
//        RainFallConsumer().Consume()
//        RiverLevelConsumer().Consume()

        if (args[0].toInt() == DAMLEVEL) DamLevelConsumer().Consume()
        if (args[0].toInt() == FLOODGATE) FloodGateConsumer().Consume()
        if (args[0].toInt() == RAINFALL) RainFallConsumer().Consume()
        if (args[0].toInt() == RIVERLEVEL) RiverLevelConsumer().Consume()
    }
}